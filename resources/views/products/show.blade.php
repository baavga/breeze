...
<link href={{asset("css/bootstrap.min.css")}} rel="stylesheet">
 @section('content')  
	<!-- The four columns --> 
	<br><a href="/барилгын-материал-зарна/{{$product->category->slug}}" class = 'btn btn-dark'>{{$product->category->name}}</a><br>
 
	<div class="row">
		
		<div class="column" style="margin-left:5px !important;">
			 @if(count($product->image1)>0)
		<img class = "rounded border border-secondary" src="/storage/{{$product->image1}}" alt="{{$product->name}}" style="width:50px;height:50px" onclick="myFunction(this);">
				 @endif
			 @if(count($product->image2)>0)
				 <img class = "rounded border border-secondary" src="/storage/{{$product->image2}}" alt="{{$product->name}}" style="width:50px;height:50px" onclick="myFunction(this);">
				 @endif
			 @if(count($product->image3)>0)
				 <img class = "rounded border border-secondary" src="/storage/{{$product->image3}}" alt="{{$product->name}}" style="width:50px;height:50px" onclick="myFunction(this);">
				 @endif
		</div> 
	</div> 
